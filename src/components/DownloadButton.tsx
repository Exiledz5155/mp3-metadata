import { Button, Box, AbsoluteCenter, Container, Stack, Heading} from '@chakra-ui/react'
        ...
        <Container centerContent>
            <Stack>
                ...
                <Box bg='green.100' padding={"10px"}>
                    {files.map((f) => (
                        <Box key={f} bg='green.400' textAlign={"center"} padding={"10px"}>{f.split("/").pop()}</Box> // display all the files to be zipped
                    ))}
                </Box>
                <Box position='relative' h='100px'>
                    <AbsoluteCenter bg='green.100' p='4' color='white' axis='both'>
                        <Button onClick={()=> downloadFileAtURL(test)} justifyContent={"center"}>Download Zip</Button>
                    </AbsoluteCenter>
                </Box>
            </Stack>
        </Container>
