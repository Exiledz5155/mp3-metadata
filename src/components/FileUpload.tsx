...
async function uploadFile(file: File, userUUID: string): Promise<Response> {
  ...
  const response = await fetch(`/api/generateSasToken?fileName=${fileName}&userUUID=${userUUID}`);
  ...
  uploadedFiles.push({ fileName: file.name, timestamp: new Date().toISOString() });
